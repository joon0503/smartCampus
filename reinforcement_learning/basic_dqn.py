...
class QAgent:
    ...
    def __init__(self, options, name):
        ...
        with tf.variable_scope(self.scope):  # Set variable scope
            ...
            self.observation = tf.placeholder(tf.float32, [None, (SENSOR_COUNT + 2) * options.FRAME_COUNT],
                                              name='observation')
            ...
            if options.disable_duel == True:
                ...
                self.output = self.val_est + tf.subtract(self.adv_est,
                                                         tf.reduce_mean(self.adv_est, axis=1, keepdims=True))
                ######################################:
    ...
    def getTrainableVarByName(self):
        ...
        trainable_vars_by_name = {var.name[len(self.scope):]: var for var in
                                  trainable_vars}  # This makes a dictionary with key being the name of varialbe without scope
...
def getVehicleStateLUA():
    ...
    res, retInts, retFloats, retStrings, retBuffer = vrep.simxCallScriptFunction(clientID, 'remoteApiCommandServer',
                                                                                 ...
                                                                                 'getVehicleState_function',
                                                                                 handle_list, [], [], emptyBuff,
                                                                                 vrep.simx_opmode_blocking)

    # Unpack Data
    out_data = np.reshape(retFloats, (
    -1, 18))  # Reshape such that each row is 18 elements. 3 for pos, 3 for ori, 9 for sensor, 3 for goal pos

    return out_data[:, 0:2], ((out_data[:, 5] - math.pi * 0.5) / (math.pi / 2)), out_data[:,
                                                                                 6:6 + SENSOR_COUNT] / MAX_DISTANCE, np.transpose(
        getGoalInfo(out_data[:, 0:2], out_data[:, 15:17]))
...
def initScene(veh_index_list, randomize=False):
    ...
    for veh_index in veh_index_list:
        vrep.simxSetModelProperty(clientID, vehicle_handle[veh_index], vrep.sim_modelproperty_not_dynamic,
                                  vrep.simx_opmode_blocking)  # Disable dynamic
        # Reset position of vehicle. Randomize x-position if enabled
        if randomize == False:
            err_code = vrep.simxSetObjectPosition(clientID, vehicle_handle[veh_index], -1, [veh_index * 20 + 0, 0, 0.2],
                                                  vrep.simx_opmode_blocking)
        else:
            x_pos = random.uniform(-1, -3)
            while True:
                y_pos = random.uniform(0, 50)
                if not 25 <= y_pos <= 35:
                    break

            err_code = vrep.simxSetObjectPosition(clientID, vehicle_handle[veh_index], -1,
                                                  [veh_index * 20 + x_pos, y_pos, 0.2], vrep.simx_opmode_blocking)

        # Reset Orientation of vehicle
        err_code = vrep.simxSetObjectOrientation(clientID, vehicle_handle[veh_index], -1, [0, 0, math.radians(90)],
                                                 vrep.simx_opmode_blocking)
    ...
    for veh_index in veh_index_list:
        ...
        vrep.simxSetObjectFloatParameter(clientID, vehicle_handle[veh_index], vrep.sim_shapefloatparam_init_velocity_y,
                                         init_speed * (1000 / 3600), vrep.simx_opmode_blocking)

        # Read sensor
        dState, dDistance = readSensor(clientID, sensor_handle[veh_index],
                                       vrep.simx_opmode_buffer)  # try it once for initialization

        # Reset position of obstacle
        if randomize == True:
            if random.random() > 0.5:
                err_code = vrep.simxSetObjectPosition(clientID, obs_handle[veh_index], -1,
                                                      [veh_index * 20 + -3.3, 30, 1.1], vrep.simx_opmode_blocking)
            else:
                err_code = vrep.simxSetObjectPosition(clientID, obs_handle[veh_index], -1,
                                                      [veh_index * 20 + 1.675, 30, 1.1], vrep.simx_opmode_blocking)
...
if __name__ == "__main__":
    ...
    for i in range(0, VEH_COUNT):
        err_code, vehicle_handle[i] = vrep.simxGetObjectHandle(clientID, "dyros_vehicle" + str(i),
                                                               vrep.simx_opmode_blocking)
    ...
    action_stack = np.zeros(
        (VEH_COUNT, options.ACTION_DIM))  # action_stack[k] is the array of optinos.ACTION_DIM with one hot encoding
    ...
    while epi_counter <= options.MAX_EPISODE:
        ...
        for v in range(0, VEH_COUNT):
            ...
            if detectCollision(next_dDistance[v])[0] == True:
                ...
                # Print last Q-value
                observation = getObs(sensor_queue[v], goal_queue[v], old=False)
                curr_q_value = sess.run([agent_train.output],
                                        feed_dict={agent_train.observation: np.reshape(observation, (1, -1))})
                print(curr_q_value)
        ...
        if global_step >= options.MAX_EXPERIENCE and options.TESTING == False:
            for tf_train_counter in range(0, VEH_COUNT):
                ...
                if options.disable_DN == False:
                    ...
                    q_target_val = rewards_mb + options.GAMMA * agent_target.output.eval(feed_dict=feed)[
                        np.arange(0, options.BATCH_SIZE), action_train]
                else:
                    feed.clear()
                    feed.update({agent_target.observation: next_states_mb})
                    # Just using Target Network
                    q_target_val = rewards_mb + options.GAMMA * np.amax(agent.target.output.eval(feed_dict=feed),
                                                                        axis=1)
                ...
                with tf.variable_scope("Training"):
                    step_loss_per_data, step_loss_value, _ = sess.run(
                        [agent_train.loss_per_data, agent_train.loss, agent_train.optimizer], feed_dict=feed)
        ...
        if options.TESTING == False:
            if global_step // options.SAVER_RATE >= 1 and global_step % options.SAVER_RATE == 0 and options.NO_SAVE == False:
                ...
                saver.save(sess,
                           'checkpoints-vehicle/vehicle-dqn_s' + START_TIME + "_e" + str(epi_counter) + "_gs" + str(
                               global_step))
                print("Done")
