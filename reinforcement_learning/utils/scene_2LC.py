    ...
    for veh_index in veh_index_list:
        ...
        if randomize == True:
            if random.random() > 0.5:
                err_code = vrep.simxSetObjectPosition(scene_const.clientID,obs_handle[veh_index],-1,[veh_index*scene_const.case_width + 0.5*(1-scene_const.obs_w)*scene_const.lane_width,0,1.1],vrep.simx_opmode_blocking)
            else:
                err_code = vrep.simxSetObjectPosition(scene_const.clientID,obs_handle[veh_index],-1,[veh_index*scene_const.case_width - 0.5*(1-scene_const.obs_w)*scene_const.lane_width,0,1.1],vrep.simx_opmode_blocking)
