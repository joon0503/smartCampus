...
#    err_code,vehicle_handle = vrep.simxGetObjectHandle(clientID,"dyros_vehicle0", vrep.simx_opmode_blocking) 
    ...
    createObject([5, 150, 0.1],[-1.25 + i*20,0,-0.1],'floor' + str(i))                  # create floor
    createObject([0.1, 150, 2.5],[-3.75 + i*20,0,1.25],'wallLeft' + str(i))             # create wall left
    createObject([0.1, 150, 2.5],[1.25 + i*20,0,1.25],'wallRight' + str(i))             # create wall right
    createObject([2.2, 2.2 , 2],[0 + i*20,30, 1.1],'obstacle' + str(i))                 # create obstacle
    createObject([5, 0.2 , 2.5],[-1.25 + i*20,70, 1.25],'wallEnd' + str(i))                 # create end wall
    createObject([5, 0.2 , 2.5],[-1.25 + i*20,-10, 1.25],'wallBack' + str(i))                 # create end wall
