...
from utils.a2c_algorithm import a2c
...
if __name__ == "__main__":
    ...
    while epi_counter <= options.MAX_EPISODE:
        ...
        for v in range(0,options.VEH_COUNT):
            ...
            # Save new memory 
            a2c_algo.replay_memory.store(experience)
        ...
        if global_step >= options.MAX_EXPERIENCE and options.TESTING == False:
            for tf_train_counter in range(0,options.VEH_COUNT):
                ...
                ##############################
                loss_k, states_sensor_mb, next_states_sensor_mb, states_goal_mb, next_states_goal_mb, actions_mb = a2c_algo.trainOneStep()
        ...
        handle_dict, sim_env.scene_const, case_direction = sim_env.initScene( reset_veh_list, RANDOMIZE, a2c_algo.course_eps )

        ###############
        # Miscellaneous
        ###############
        a2c_algo.updateMiscellaneous( global_step )
        ...
        for v in reset_veh_list:
            ...
            print('\tEPS             : ' + str(a2c_algo.eps))
            print('\tCourse EPS      : ' + str(a2c_algo.course_eps))
        ...
        for v in reset_veh_list:
            ...
            data_package.add_eps( a2c_algo.eps )                     # Add epsilon used for this reward
        ...
        if options.TESTING == False:
            if options.NO_SAVE == False and epi_counter - last_saved_epi >= options.SAVER_RATE:
                a2c_algo.saveNetworkKeras(START_TIME_STR, epi_counter, global_step)
                print('-----------------------------------------')
                print("Saving data...") 
                print('-----------------------------------------')
